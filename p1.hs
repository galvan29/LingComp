...
diagonal (Mat n (Q x1 x2 x3 x4)) = if(dNZero (Mat n (Q x1 x2 x3 x4))) && (uLZero (Mat n (Q x1 x2 x3 x4))) then True else False
...
dToList (Mat n (Q x1 x2 x3 x4)) = if n>0 then ((dToList (Mat (n-1) x1)) ++ (dToList (Mat (n-1) x4)))
